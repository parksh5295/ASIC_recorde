...
from Signature_tool.signature_reduction import reduce_signatures_by_subsets, remove_inactive_signatures
...
def main(args):
    ...
    try:
        ...
        for chunk in main_chunk_iterator:
            ...
            # --- Remove blacklisted signatures (FP signatures) from all_valid_signatures ---
            # This must happen BEFORE reduction and inactive removal to prevent them from using signatures that will be removed
            if signatures_to_remove:
                removed_count = len(all_valid_signatures)
                for sig_id in list(signatures_to_remove):
                    if sig_id in all_valid_signatures:
                        del all_valid_signatures[sig_id]
                        if sig_id in signature_turn_created:
                            del signature_turn_created[sig_id]
                removed_count = removed_count - len(all_valid_signatures)
                if removed_count > 0:
                    logger.info(f"[FP Removal] Removed {removed_count} blacklisted signatures from all_valid_signatures.")
                    # Clear signatures_to_remove after removal (they're already removed)
                    signatures_to_remove.clear()

            # --- 2. Remove Inactive Signatures (before Reduction) ---
            # Remove signatures that never triggered alerts on the current turn's data
            # This must happen BEFORE reduction to prevent reduction from using inactive signatures
            
            # --- DEBUG: Evaluate performance BEFORE inactive removal (for comparison) ---
            pre_inactive_recall, pre_inactive_precision, pre_inactive_f1, pre_inactive_accuracy = 0, 0, 0, 0
            if all_valid_signatures and turn_eval_data is not None and not turn_eval_data.empty:
                pre_inactive_sigs = sorted(all_valid_signatures.items(), key=lambda x: x[0])
                formatted_pre_inactive_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} for sid, r in pre_inactive_sigs]
                _, _, pre_inactive_recall, pre_inactive_precision, pre_inactive_f1, pre_inactive_accuracy, _ = calculate_performance_in_batches(
                    turn_eval_data, 
                    formatted_pre_inactive_sigs, 
                    args.evaluation_batch_size
                )
                logger.debug(f"[InactiveRemoval Pre-Eval] Before inactive removal: Recall={pre_inactive_recall:.6f}, Precision={pre_inactive_precision:.6f}, "
                           f"F1={pre_inactive_f1:.6f}, Accuracy={pre_inactive_accuracy:.6f}, Signatures={len(all_valid_signatures)}")
            
            inactive_removed_count = 0
            inactive_removed_from_new = 0
            count_before_inactive = len(all_valid_signatures)
            if all_valid_signatures and turn_eval_data is not None and not turn_eval_data.empty:
                # Track which signatures were created THIS turn (for graph display)
                new_signatures_before_inactive = {sig_id for sig_id in all_valid_signatures.keys() 
                                                 if signature_turn_created.get(sig_id) == turn_counter}
                
                # Evaluate signatures against turn_eval_data to find which ones are active
                # Sort by signature ID to ensure consistent ordering (for "first match wins" logic)
                sorted_sigs_for_inactive = sorted(all_valid_signatures.items(), key=lambda x: x[0])
                ...
                if inactive_eval_alerts.empty or 'signature_id' not in inactive_eval_alerts.columns:
                    # No alerts generated - all signatures are inactive
                    if inactive_eval_alerts.empty:
                        logger.warning(f"[InactiveRemoval] No alerts generated. All {len(all_valid_signatures)} signatures are inactive.")
                        # Remove all signatures (they never triggered alerts)
                        for sig_id in list(all_valid_signatures.keys()):
                            if sig_id in signature_turn_created:
                                if sig_id in new_signatures_before_inactive:
                                    inactive_removed_from_new += 1
                                del signature_turn_created[sig_id]
                        inactive_removed_count = len(all_valid_signatures)
                        all_valid_signatures.clear()
                    else:
                        logger.warning("[InactiveRemoval] Invalid alert format. Skipping inactive removal.")
                else:
                    ...
                    all_sig_ids = set(all_valid_signatures.keys())
                    inactive_ids = all_sig_ids - active_sig_ids
                    
                    if inactive_ids:
                        # Remove from all_valid_signatures and signature_turn_created
                        for inactive_id in inactive_ids:
                            if inactive_id in all_valid_signatures:
                                del all_valid_signatures[inactive_id]
                                inactive_removed_count += 1
                ...
                # --- DEBUG: Evaluate performance AFTER inactive removal (for comparison) ---
                post_inactive_recall, post_inactive_precision, post_inactive_f1, post_inactive_accuracy = 0, 0, 0, 0
                if all_valid_signatures and turn_eval_data is not None and not turn_eval_data.empty:
                    post_inactive_sigs = sorted(all_valid_signatures.items(), key=lambda x: x[0])
                    formatted_post_inactive_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} for sid, r in post_inactive_sigs]
                    _, _, post_inactive_recall, post_inactive_precision, post_inactive_f1, post_inactive_accuracy, _ = calculate_performance_in_batches(
                        turn_eval_data, 
                        formatted_post_inactive_sigs, 
                        args.evaluation_batch_size
                    )
                    logger.debug(f"[InactiveRemoval Post-Eval] After inactive removal: Recall={post_inactive_recall:.6f}, Precision={post_inactive_precision:.6f}, "
                               f"F1={post_inactive_f1:.6f}, Accuracy={post_inactive_accuracy:.6f}, Signatures={len(all_valid_signatures)}")
                    
                    # Calculate differences
                    recall_diff = post_inactive_recall - pre_inactive_recall
                    precision_diff = post_inactive_precision - pre_inactive_precision
                    f1_diff = post_inactive_f1 - pre_inactive_f1
                    accuracy_diff = post_inactive_accuracy - pre_inactive_accuracy
                    
                    logger.debug(f"[InactiveRemoval Impact] Differences (Post - Pre): "
                               f"Recall={recall_diff:+.6f}, Precision={precision_diff:+.6f}, "
                               f"F1={f1_diff:+.6f}, Accuracy={accuracy_diff:+.6f}")
                
                count_after_inactive = len(all_valid_signatures)
                if inactive_removed_count > 0:
                    logger.info(f"[InactiveRemoval] Removed {inactive_removed_count} inactive signatures that never triggered alerts on turn data. "
                              f"Signatures: {count_before_inactive} -> {count_after_inactive}")
                else:
                    logger.debug(f"[InactiveRemoval] No inactive signatures found. All {count_before_inactive} signatures are active.")

            # --- 3. Signature Reduction (before Exit Evaluation) ---
            # IMPORTANT: This must happen AFTER all other filtering/removal steps (FP, Pruning, etc.)
            # to prevent reduction from using signatures that are marked for removal
            # Remove supersets when subsets exist (e.g., {a:1,b:3,c:6} and {a:1,c:6} -> keep only {a:1,c:6})
            
            # --- DEBUG: Evaluate performance BEFORE reduction (for comparison) ---
            pre_reduction_recall, pre_reduction_precision, pre_reduction_f1, pre_reduction_accuracy = 0, 0, 0, 0
            if all_valid_signatures and turn_eval_data is not None and not turn_eval_data.empty:
                pre_reduction_sigs = sorted(all_valid_signatures.items(), key=lambda x: x[0])
                formatted_pre_reduction_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} for sid, r in pre_reduction_sigs]
                _, _, pre_reduction_recall, pre_reduction_precision, pre_reduction_f1, pre_reduction_accuracy, _ = calculate_performance_in_batches(
                    turn_eval_data, 
                    formatted_pre_reduction_sigs, 
                    args.evaluation_batch_size
                )
                logger.debug(f"[Reduction Pre-Eval] Before reduction: Recall={pre_reduction_recall:.6f}, Precision={pre_reduction_precision:.6f}, "
                           f"F1={pre_reduction_f1:.6f}, Accuracy={pre_reduction_accuracy:.6f}, Signatures={len(all_valid_signatures)}")
            
            reduction_count_before = len(all_valid_signatures)
            ids_before_reduction = set(all_valid_signatures.keys())
            # Track which signatures were created THIS turn (for graph display)
            new_signatures_this_turn = {sig_id for sig_id in all_valid_signatures.keys() 
                                       if signature_turn_created.get(sig_id) == turn_counter}
            all_valid_signatures = reduce_signatures_by_subsets(all_valid_signatures)
            reduction_count_after = len(all_valid_signatures)
            reduction_removed = reduction_count_before - reduction_count_after
            reduction_removed_from_new = 0  # Count only from newly created signatures this turn
            if reduction_removed > 0:
                logger.info(f"[Reduction] Removed {reduction_removed} superset signatures (before Exit eval). "
                          f"Signatures: {reduction_count_before} -> {reduction_count_after}")
                # Remove reduced signatures from signature_turn_created
                ids_after_reduction = set(all_valid_signatures.keys())
                removed_ids = ids_before_reduction - ids_after_reduction
                for removed_id in removed_ids:
                    if removed_id in signature_turn_created:
                        # Count if this was a newly created signature this turn
                        if removed_id in new_signatures_this_turn:
                            reduction_removed_from_new += 1
                        del signature_turn_created[removed_id]
            
            # --- DEBUG: Evaluate performance AFTER reduction (for comparison) ---
            post_reduction_recall, post_reduction_precision, post_reduction_f1, post_reduction_accuracy = 0, 0, 0, 0
            if all_valid_signatures and turn_eval_data is not None and not turn_eval_data.empty:
                post_reduction_sigs = sorted(all_valid_signatures.items(), key=lambda x: x[0])
                formatted_post_reduction_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} for sid, r in post_reduction_sigs]
                _, _, post_reduction_recall, post_reduction_precision, post_reduction_f1, post_reduction_accuracy, _ = calculate_performance_in_batches(
                    turn_eval_data, 
                    formatted_post_reduction_sigs, 
                    args.evaluation_batch_size
                )
                logger.debug(f"[Reduction Post-Eval] After reduction: Recall={post_reduction_recall:.6f}, Precision={post_reduction_precision:.6f}, "
                           f"F1={post_reduction_f1:.6f}, Accuracy={post_reduction_accuracy:.6f}, Signatures={len(all_valid_signatures)}")
                
                # Calculate differences
                recall_diff = post_reduction_recall - pre_reduction_recall
                precision_diff = post_reduction_precision - pre_reduction_precision
                f1_diff = post_reduction_f1 - pre_reduction_f1
                accuracy_diff = post_reduction_accuracy - pre_reduction_accuracy
                
                logger.debug(f"[Reduction Impact] Differences (Post - Pre): "
                           f"Recall={recall_diff:+.6f}, Precision={precision_diff:+.6f}, "
                           f"F1={f1_diff:+.6f}, Accuracy={accuracy_diff:+.6f}")

            # --- 4. Exit Performance Evaluation ---
            # Note: All removals (FP, Inactive, Reduction) are complete at this point
            # Exit evaluation is performed on the final signature set (after all removals)
            rules_at_turn_end = {sig_id: rule for sig_id, rule in all_valid_signatures.items()}
            ...
            if rules_at_turn_end and turn_eval_data is not None and not turn_eval_data.empty:
                # Sort by signature ID to ensure consistent ordering (independent of reduction)
                # This prevents "first match wins" from producing different results due to order changes
                sorted_sigs = sorted(rules_at_turn_end.items(), key=lambda x: x[0])
                formatted_exit_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} for sid, r in sorted_sigs]
            ...
            logger.info(f"End of Turn {turn_counter}. Signatures: {entry_sig_count} -> {exit_sig_count} "
                       f"(reduced: {reduction_removed}, inactive removed: {inactive_removed_count}). "
                       f"EXIT Recall: {exit_recall:.4f}. EXIT Precision: {exit_precision:.4f}. "
                       f"EXIT F1: {exit_f1:.4f}. EXIT Accuracy: {exit_accuracy:.4f}")
            ...
            history.append({
                ...
                'removed': newly_removed_count,  # Only FP + pruning removals (NOT reduction/inactive)
                'reduction_removed': reduction_removed,  # Total reduction removals (for logging)
        ...
        # --- Finalization ---
        # Note: signatures_to_remove should be empty at this point (already removed before reduction)
        ...
        logger.info(f"Signatures removed due to FPs: {len(signatures_to_remove) if signatures_to_remove else 0}")
        logger.info(f"Final count before final reduction/inactive removal: {len(final_signatures)}")

        # --- Final Reduction and Inactive Removal (before attack type extraction) ---
        # This reduces the number of signatures for which we need to extract attack types
        logger.info("--- Applying final signature reduction and inactive removal ---")
        
        # Final reduction: remove supersets when subsets exist
        reduction_count_before_final = len(final_signatures)
        final_signatures = reduce_signatures_by_subsets(final_signatures)
        ...
        # Final inactive removal: remove signatures that never triggered alerts in the final evaluation
        # We need to evaluate all final signatures against the entire dataset to find inactive ones
        if final_signatures and chunk_cache is not None and not chunk_cache.is_empty():
            logger.info(f"Evaluating {len(final_signatures)} final signatures to identify inactive ones...")
            formatted_final_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} 
                                   for sid, r in final_signatures.items()]
            ...
            inactive_count_before_final = len(final_signatures)
            if not final_eval_alerts.empty and 'signature_id' in final_eval_alerts.columns:
                ...
                all_sig_ids = set(final_signatures.keys())
                inactive_ids = all_sig_ids - active_sig_ids
                
                if inactive_ids:
                    # Remove inactive signatures
                    for inactive_id in inactive_ids:
                        if inactive_id in final_signatures:
                            del final_signatures[inactive_id]
                            # Also remove from signature_turn_created for consistency
                            if inactive_id in signature_turn_created:
                                del signature_turn_created[inactive_id]
                    
                    inactive_count_after_final = len(final_signatures)
                ...
                logger.warning("[Final Inactive Removal] No alerts generated or invalid alert format. Skipping inactive removal.")
        ...
        def extract_attack_types_from_alerts(alerts_df, chunk_cache_obj, attack_cols, batch_size):
            """
            Extract attack types for all signatures using pre-computed alerts.
            This avoids re-streaming the entire dataset for each signature.
            """
            if alerts_df.empty or 'signature_id' not in alerts_df.columns:
                # Fallback: return empty attack types for all signatures
                return {sig_id: set(['-']) for sig_id in final_signatures.keys()}
            
            # Group alerts by signature_id and collect alert_indices
            sig_to_indices = {}
            for sig_id in alerts_df['signature_id'].dropna().unique():
                sig_alerts = alerts_df[alerts_df['signature_id'] == sig_id]
                sig_to_indices[sig_id] = set(sig_alerts['alert_index'].dropna().unique())
            
            # Initialize attack types for all signatures
            sig_to_attacks = {sig_id: set() for sig_id in final_signatures.keys()}
            
            if not attack_cols:
                # No attack columns to extract
                for sig_id in sig_to_attacks:
                    if not sig_to_attacks[sig_id]:
                        sig_to_attacks[sig_id].add('-')
                return sig_to_attacks
            
            # Stream through chunks once and extract attack types for matched rows
            logger.info(f"Extracting attack types from pre-computed alerts (one-time streaming)...")
            matched_indices_processed = set()
            
            for batch in chunk_cache_obj.iter_batches(batch_size=batch_size):
                if batch is None or batch.empty:
                    continue
                
                # Find which signatures have matches in this batch
                batch_indices = set(batch.index)
                
                for sig_id, alert_indices in sig_to_indices.items():
                    if sig_id not in sig_to_attacks:
                        continue
                    
                    # Find matching rows in this batch
                    matching_indices = alert_indices.intersection(batch_indices)
                    if not matching_indices:
                        continue
                    
                    # Get matching rows
                    matched_rows = batch.loc[list(matching_indices)]
                    ...
                    if attack_cols == ['reconnaissance', 'infection', 'action']:
                        for col in attack_cols:
                            if col in matched_rows.columns:
                                ...
                                if (vals == 1).any():
                                    sig_to_attacks[sig_id].add(col)
                        continue
                    
                    # General attack column handling
                    for col in attack_cols:
                        if col in matched_rows.columns:
                            vals = matched_rows[col].astype(str).unique().tolist()
                            sig_to_attacks[sig_id].update(vals)
                    
                    matched_indices_processed.update(matching_indices)
            
            # Fill in '-' for signatures with no attacks found
            for sig_id in sig_to_attacks:
                if not sig_to_attacks[sig_id]:
                    sig_to_attacks[sig_id].add('-')
            
            # Also handle signatures that never triggered alerts (won't be in sig_to_indices)
            for sig_id in final_signatures.keys():
                if sig_id not in sig_to_indices:
                    if not sig_to_attacks[sig_id]:
                        sig_to_attacks[sig_id].add('-')
        ...
        # Use optimized extraction from pre-computed alerts (if available)
        if 'final_eval_alerts' in locals() and not final_eval_alerts.empty:
            logger.info("Using pre-computed alerts to extract attack types (optimized)...")
            sig_to_attacks = extract_attack_types_from_alerts(
                final_eval_alerts, 
                chunk_cache, 
                attack_cols, 
                args.evaluation_batch_size
            ...
            for sig_idx, (sig_id, rule) in enumerate(final_signatures.items(), 1):
                ...
                attacks = sig_to_attacks.get(sig_id, set(['-']))
            ...
            # Fallback: original method (if alerts not available)
            logger.warning("Pre-computed alerts not available. Using fallback method (slower)...")
        ...
        logger.info(f"Finished extracting attack types for all {total_sigs} signatures.")
