        ...
        for chunk in main_chunk_iterator:
            ...
            try:
                ...
                if rule_spooler.has_rules():
                    ...
                    use_batched_normal = False
                    normal_data_sub_chunks = None

                    if not normal_data_empty and args.normal_data_batch_size and len(normal_data_in_chunk) > args.normal_data_batch_size:
                        use_batched_normal = True
                        logger.info(f"Splitting normal data of size {len(normal_data_in_chunk)} into batches of {args.normal_data_batch_size}.")
                        normal_data_sub_chunks = [
                    ...
                    def filter_rule_batch(rule_batch):
                        ...
                        filtered = []
                        if use_batched_normal:
                            processes_for_filter = max(1, min(args.num_processes, len(rule_batch)))
                            ...
                            with multiprocessing.Pool(
                                ...
                                initargs=(normal_data_sub_chunks, normal_min_support, negative_filtering_enabled, negative_filter_threshold)
                            ) as pool:
                            ...
                            with multiprocessing.Pool(
                                processes=processes_for_filter,
                                initializer=init_worker_filter,
                                initargs=(normal_data_in_chunk, normal_min_support, negative_filtering_enabled, negative_filter_threshold)
                            ) as pool:
                                results_iterator = pool.imap_unordered(is_rule_valid_for_filtering, rule_batch, chunksize=chunksize)
                                for result_rule in results_iterator:
                                    if result_rule is not None:
                                        filtered.append(result_rule)
                        return filtered
