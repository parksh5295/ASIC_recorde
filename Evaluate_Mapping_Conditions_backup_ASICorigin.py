...
def run_evaluation_for_condition(
    ...
    # --- NEW: Regroup the fine-grained signatures into conceptual groups FIRST ---
    # This step is crucial. It consolidates multiple fine-grained rules into a single coarse-grained rule.
    # We use a set to automatically handle duplicates.
    
    # 1. Regroup training signatures
    regrouped_train_signatures_set = set()
    for sig in signatures_from_train_100:
        regrouped_sig = {}
        for key, value in sig.items():
            if key in current_regrouping_map:
                # FIX: Convert value to float then int to handle various numeric types ('3.0', 3.0, 3, '3')
                try:
                    #int_value = int(value)
                    int_value = int(float(value))
                    regrouped_sig[key] = current_regrouping_map[key].get(int_value, int_value)
                except (ValueError, TypeError):
                    #regrouped_sig[key] = value # Keep original if not an int
                    # If conversion fails, log it once per run for debugging and keep original.
                    if not getattr(run_evaluation_for_condition, 'has_logged_type_error', False):
                        print(f"[DEBUG] Could not convert value '{value}' (type: {type(value)}) to int for regrouping. Keeping original.")
                        run_evaluation_for_condition.has_logged_type_error = True
    ...
    # The dataframe for checking support is still the fine-grained (0-99) one.
    # calculate_support_for_itemset will handle the comparison correctly.
    ...
    if 'original_label' in test_df_base.columns:
        ...
        print(f"[4/9] Filtering pre-generated TEST signatures using VIRTUAL labels and regrouping map...")

        # 2. Regroup test signatures (only if they exist)
        regrouped_test_signatures_set = set()
        
        # --- REMOVED redundant TEST signature debug block ---

        for sig in signatures_from_test_100:
            regrouped_sig = {}
            for key, value in sig.items():
                if key in current_regrouping_map:
                    # FIX: Convert value to float then int
                    try:
                        #int_value = int(value)
                        int_value = int(float(value))
                        regrouped_sig[key] = current_regrouping_map[key].get(int_value, int_value)
        ...
        # We use the base_mapped_test data which has VIRTUAL labels
    ...
    # --- NEW 7. Remove Inactive Signatures ---
    # Removes signatures that do not fire at all on the test set (neither TP nor FP).
    stage7_start = time.time()
    print(f"[7/9] Removing inactive signatures that do not fire on the test set...")
    
    # --- FIX: Use the final evaluation dataframe for the activity check ---
    # This ensures consistency, as we are checking for activity on the exact same data
    # that will be used for the final performance calculation.
    # We prepare it here, just as it would be for stage 8.
    if 'original_label' in test_df_base.columns:
        activity_check_df = base_mapped_test.drop(columns=['label', 'original_label'], errors='ignore').copy()
        activity_check_df['label'] = test_df_base['original_label'].values
    else:
        activity_check_df = base_mapped_test

    active_signatures = [
        rule for rule in final_signatures
        if calculate_support_for_itemset(rule, activity_check_df, regrouping_map=current_regrouping_map) > 0
    ]
    print(f"-> {len(active_signatures)} active signatures remain out of {len(final_signatures)}.")
    final_signatures = active_signatures # This is the final-final set
