...
def preprocess_and_map_chunk(chunk_df, file_type, category_mapping, data_list, n_splits=None):
    """
    ...
    mapped_chunk, _ = map_intervals_to_groups(chunk_embedded, category_mapping, chunk_data_list, 'N')
...
def main(args):
    ...
    try:
        ...
        run_dir = os.path.join("../Dataset_ISV_wholemap", args.file_type, params_str) # Used for artifacts and checkpoints
        ...
        # --- NEW: Scan entire dataset to compute min/max and quantile positions for mapping generation ---
        logger.info("--- Scanning entire dataset to compute feature ranges and quantile positions for mapping ---")
        label_cols_to_exclude = get_label_columns_to_exclude(args.file_type)
        feature_mins = {}
        feature_maxs = {}
        feature_quantiles = {}  # Store quantile values for each feature
        scan_chunk_count = 0
        scan_total_rows = 0
        
        try:
            scan_iterator = pd.read_csv(file_path, chunksize=args.chunk_size, low_memory=False)
            # Collect all numeric values for quantile calculation
            all_numeric_values = {}  # {col_name: list of all values}
            ...
            for scan_chunk in scan_iterator:
                ...
                # Update min/max and collect values for quantile calculation
            ...
            logger.info("Calculating quantile positions for all numeric features...")
            n_splits = args.n_splits
            quantile_percentiles = [i / n_splits * 100 for i in range(n_splits + 1)]  # 0%, 2.5%, 5%, ..., 100% for n_splits=40
            
            for col, values_list in all_numeric_values.items():
                try:
                    values_array = np.array(values_list)
                    quantiles = np.percentile(values_array, quantile_percentiles)
                    feature_quantiles[col] = quantiles
                    logger.debug(f"  {col}: quantiles calculated ({len(quantiles)} positions)")
                except Exception as e:
                    logger.debug(f"  Failed to calculate quantiles for {col}: {e}")
                    continue
            
            logger.info(f"Scan complete: {scan_chunk_count} chunks ({scan_total_rows} rows), {len(feature_mins)} numeric features found.")
            logger.info(f"Quantile positions calculated for {len(feature_quantiles)} features.")
            if feature_mins:
                ...
                if feature_quantiles:
                    logger.debug(f"Sample quantile info (first feature): {list(feature_quantiles.keys())[0]} has {len(feature_quantiles[list(feature_quantiles.keys())[0]])} quantile positions")
            
            # Free memory
            del all_numeric_values
            gc.collect()
        except Exception as e:
        ...
        # --- NEW: If we scanned the entire dataset, add quantile positions to first chunk ---
        # Strategy: Add quantile position values to first chunk data to ensure pd.qcut uses full distribution
        # This ensures pd.qcut sees the full distribution without distorting the data
        if feature_quantiles:
            logger.info("Adding full dataset quantile positions to first chunk for mapping generation...")
            
            # Create a copy of first chunk data
            mapping_features_expanded = mapping_features.copy()
            
            # For each numeric feature, add rows with quantile values
            rows_added = 0
            base_row = mapping_features.iloc[0].copy() if len(mapping_features) > 0 else pd.Series()
            
            for col, quantiles in feature_quantiles.items():
                if col in mapping_features.columns:
                    try:
                        numeric_col = pd.to_numeric(mapping_features[col], errors='coerce').dropna()
                        if not numeric_col.empty:
                            # Add quantile values that are not already present in the first chunk
                            existing_values = set(numeric_col.values)
                            
                            for q_val in quantiles:
                                # Only add if significantly different from existing values
                                if not any(abs(existing_val - q_val) < 1e-6 for existing_val in existing_values):
                                    quantile_row = base_row.copy()
                                    quantile_row[col] = q_val
                                    mapping_features_expanded = pd.concat([mapping_features_expanded, quantile_row.to_frame().T], ignore_index=True)
                                    rows_added += 1
                                    existing_values.add(q_val)
                    except Exception as e:
                        logger.debug(f"  Failed to add quantiles for column {col}: {e}")
                        continue
            
            mapping_features = mapping_features_expanded
            logger.info(f"Added {rows_added} quantile position rows to ensure full distribution coverage ({len(mapping_features)} total rows).")
        elif feature_mins and feature_maxs:
            # Fallback to min/max if quantiles not available
            logger.info("Quantiles not available, adding full dataset min/max bounds to first chunk for mapping generation...")
        ...
        embedded_dataframe, _, category_mapping, data_list = choose_heterogeneous_method(
            mapping_features,
        ...
        try:
            ...
            csv_filename = f"{args.file_type}_{args.file_number}_mapped_info_wholemap.csv"
        ...
        if resume_from_turn > 0:
            ...
            for chunk in main_chunk_iterator:
                ...
                mapped_chunk = preprocess_and_map_chunk(processed_chunk, args.file_type, category_mapping, data_list, args.n_splits)
