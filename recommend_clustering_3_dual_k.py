...
def print_and_save_summary_report(run_results, est_k_surrogate, est_k_bic, file_type, file_number):
    ...
    file_type_dir = os.path.join(output_dir, file_type)
    os.makedirs(file_type_dir, exist_ok=True)
    save_path = os.path.join(file_type_dir, f"{file_type}_{file_number}_recommendation_3_dual_k_report.csv")
    df.to_csv(save_path, index=False)
...
def main():
    ...
    # For the decision tree, we'll primarily use the surrogate-based k,
    # as it's consistent with the main project's optimization goals.
    k_for_analysis = est_k_surrogate

    N_RUNS = 15
    all_run_results = []

    print("\n" + "="*70)
    print(f"Starting Analysis with {N_RUNS} Independent Runs (using k={k_for_analysis} for analysis)")
    print("="*70)
    ...
    for i in range(N_RUNS):
        ...
        q1_ratio, q1_reason = analyze_data_for_q1(X_final_np, n_clusters=k_for_analysis)
        print(f" Q1 Metric: {q1_reason}")

        q2_ratio, q2_reason = analyze_data_for_q2(X_final_np)
        print(f" Q2 Metric: {q2_reason}")

        q3_ratio, q3_reason = analyze_data_for_q3(X_final_np, n_clusters=k_for_analysis)
