...
def compute_surrogate_score(X, labels, known_normal_idx, **kwargs):
    ...
    # Extract diagnostic parameters if they exist
    algorithm = kwargs.get('algorithm')
    data_hash = kwargs.get('data_hash')
    k = kwargs.get('k')
    
    # Run diagnostics if parameters are provided
    if algorithm is not None and data_hash is not None and k is not None:
        try:
            from Dataset_Choose_Rule.surrogate_score_diagnostics import diagnose_clustering_issues
            issues_detected, issues, nan_count = diagnose_clustering_issues(X, labels, k, algorithm, data_hash)
            
            if issues_detected:
                print(f"[DIAGNOSTIC] Found {len(issues)} potential issues for {algorithm} k={k}: {[issue['type'] for issue in issues]}")
        except Exception as e:
            print(f"[DIAGNOSTIC] Error running diagnostics: {e}")
    ...
    final_score = result_dict['final']
    if np.isnan(final_score):
        # Save diagnostic information about NaN final score
        if algorithm is not None and data_hash is not None and k is not None:
            try:
                from Dataset_Choose_Rule.surrogate_score_diagnostics import save_surrogate_diagnostics
                save_surrogate_diagnostics(
                    algorithm, data_hash, k, 
                    'final_score_nan', 
                    {
                        'f1_known': result_dict['f1_known'],
                        'compact_score': result_dict['compact_score'],
                        'enrich_score': result_dict['enrich_score'],
                        'est_jaccard': result_dict['est_jaccard']
                    }
                )
            except Exception as e:
                print(f"[DIAGNOSTIC] Error saving NaN diagnostic: {e}")
        
        # Return 0.0 instead of NaN
        # final_score = 0.0  # DISABLED: Keep NaN as-is to match b35dfa0 behavior
        print(f"[WARNING] Surrogate score is NaN for {algorithm} k={k}, keeping NaN")
