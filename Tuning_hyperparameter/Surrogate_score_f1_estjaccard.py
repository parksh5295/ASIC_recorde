...
def compute_surrogate_score_optimized(X, labels, known_normal_idx, alpha=0.5, beta=0.2, gamma=0.15, delta=0.15, thresholds=[0.3,0.4,0.5,0.6,0.7,0.8,0.9], pi=0.9):
    """
    ...
    Includes estimated Jaccard score.
    """
    # Gracefully handle cases where known_normal_idx might be None
    if known_normal_idx is None:
        known_normal_idx = []
    ...
    best_f1 = 0.0 # Initialize to 0.0 to handle cases with no known normals
    best_t = None
    ...
    # --- 5. Estimated Jaccard (Optimized) ---
    '''
    p_hat = (cluster_info_df['kn'] / cluster_info_df['size']) / max(pi, 1e-6)
    p_hat = p_hat.clip(upper=1.0)
    
    # Identify benign clusters based on posterior probability > 0.5
    benign_clusters = cluster_info_df[p_hat > 0.5]
    
    if not benign_clusters.empty:
        inter = benign_clusters['kn']
        union = benign_clusters['size'] + total_kn - benign_clusters['kn']
        est_j_scores = inter / union
        est_jaccard = est_j_scores.mean()
    '''
    est_jaccard = 0.0 # Default to 0
    if total_kn > 0:
        # Create a temporary series for size to avoid modifying the original dataframe
        size_series = cluster_info_df['size'].copy()
        # HOTFIX: Prevent division by zero for empty clusters
        size_series[size_series == 0] = 1e-9 
        
        p_hat = (cluster_info_df['kn'] / size_series) / max(pi, 1e-6)
        p_hat = p_hat.clip(upper=1.0)
