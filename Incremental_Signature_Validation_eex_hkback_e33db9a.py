...
class ChunkCache:
    ...
    def register_chunk(self, df, turn_counter):
        """
...
def _yield_batches_from_source(data_source, batch_size=None):
    """
...
# --- START: New Helper functions for BATCHED normal data filtering ---
_worker_data_filter_batch = ()

def _init_worker_filter_batch(normal_data_sub_chunks, min_support):
    """Initializer for worker process, passing sub-chunks of normal data for the current turn."""
    # This prevents the worker from printing a traceback if the main process dies.
    # SIG_IGN (ignore) is more forceful than SIG_DFL (default) at suppressing the error.
    signal.signal(signal.SIGPIPE, signal.SIG_IGN)
    global _worker_data_filter_batch
    _worker_data_filter_batch = (normal_data_sub_chunks, min_support)

def _is_rule_valid_for_filtering_batch(rule):
    """Worker task that checks a rule against sub-chunks of normal data for the current turn."""
    global _worker_data_filter_batch
    normal_data_sub_chunks, min_support = _worker_data_filter_batch
    
    # A rule is a valid signature if it is NOT frequent in ANY of the normal data sub-chunks.
    for sub_chunk in normal_data_sub_chunks:
        if calculate_support_for_itemset(rule, sub_chunk, min_support):
            # The rule is frequent in this normal sub-chunk, so it's not a valid signature.
            return None
            
    # The rule was not frequent in any of the normal sub-chunks, so it is a valid signature.
    return rule
# --- END: New Helper functions for BATCHED normal data filtering ---
...
def calculate_performance_in_batches(data_source, signatures, batch_size):
    ...
    if not signatures or data_source is None:
        return 0, 0, 0, 0, 0 # tp, fp, recall, precision, f1
...
def main(args):
    ...
    try:
        ...
        params_str = get_params_str(args, max_level)
        ...
        for chunk in main_chunk_iterator:
            ...
            if rules_at_turn_start and not chunk_cache.is_empty():
                ...
                _, _, entry_recall, entry_precision, entry_f1 = calculate_performance_in_batches(
                    chunk_cache, 
            ...
            if not anomalous_data_in_chunk.empty:
                ...
                # --- NEW: Prepare args for association module, conditionally adding save_artifacts info ---
                assoc_args = {
            ...
            if anomalous_rules:
                ...
                if not normal_data_in_chunk.empty:
                    # --- NEW: Conditional Batching for Normal Data ---
                    if args.normal_data_batch_size and len(normal_data_in_chunk) > args.normal_data_batch_size:
                        logger.info(f"Splitting normal data of size {len(normal_data_in_chunk)} into batches of {args.normal_data_batch_size}.")
                        normal_data_sub_chunks = [normal_data_in_chunk.iloc[i:i + args.normal_data_batch_size] for i in range(0, len(normal_data_in_chunk), args.normal_data_batch_size)]
                        
                        with multiprocessing.Pool(processes=args.num_processes, initializer=_init_worker_filter_batch, initargs=(normal_data_sub_chunks, args.min_support)) as pool:
                            results_iterator = pool.imap_unordered(_is_rule_valid_for_filtering_batch, anomalous_rules, chunksize=1000)
                            for result_rule in results_iterator:
                                if result_rule is not None:
                                    filtered_new_rules.append(result_rule)
                    else:
                        # Original logic for non-batched normal data
                        logger.info("Processing normal data in a single batch (original method).")
            ...
            if rules_at_turn_end and not chunk_cache.is_empty():
                formatted_exit_sigs = [{'id': sid, 'name': f'Sig_{sid}', 'rule_dict': r} for sid, r in rules_at_turn_end.items()]
                
                # --- MODIFIED: Use batched performance calculation ---
                _, _, exit_recall, exit_precision, exit_f1 = calculate_performance_in_batches(
                    chunk_cache, 
                    formatted_exit_sigs, 
                    args.evaluation_batch_size
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--signature_batch_size', type=int, default=100000, help="Batch size for validating signatures to conserve memory.")
    parser.add_argument('--normal_data_batch_size', type=int, default=50000, help="Batch size for splitting the turn's normal data to conserve memory during filtering. If not set, normal data is not batched.")
    parser.add_argument('--evaluation_batch_size', type=int, default=50000, help="Batch size for processing the full dataset during final performance evaluation to conserve memory.")
