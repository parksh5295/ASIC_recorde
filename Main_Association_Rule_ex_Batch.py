...
def process_confidence_iteration(min_confidence, anomal_grouped_data, nomal_grouped_data, Association_mathod, min_support, association_metric, group_mapped_df, signature_ea, precision_underlimit, cores_per_algo_task, current_file_type, itemset_limit, batch_size,
    ...
    print(f"  [PCI Filter] Filtering {total_generated_rules:,} anomaly rules against normal data (batched for memory, global support check)...")
    signatures = []

    # Pre-split normal for memory, but accumulate global counts per rule
    ...
    def rule_passes_global(rule):
        ...
        for chunk in normal_data_chunks:
            ...
            match_mask = chunk[list(rule.keys())].eq(pd.Series(rule)).all(axis=1)
            total_matches += match_mask.sum()
        if total_rows == 0:
            return True
        support = total_matches / total_rows
        # negative filtering uses same global support as P(rule|normal)
        if negative_filtering_enabled and support > negative_filter_threshold:
            return False
        if support > normal_min_support:
            return False
        return True
...
def main():
    ...
    # Get ALL label columns to exclude (NEVER include these in signatures)
    ...
    # IMPORTANT: Exclude ALL label columns immediately to prevent label leakage
    anomal_grouped_data = anomal_grouped_data.drop(columns=label_cols_to_exclude, errors='ignore')
    nomal_grouped_data = group_mapped_df[group_mapped_df['cluster'] == 0]
    # IMPORTANT: Exclude ALL label columns immediately to prevent label leakage
    nomal_grouped_data = nomal_grouped_data.drop(columns=label_cols_to_exclude, errors='ignore')
