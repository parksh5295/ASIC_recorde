...
try:
    ...
    from Heterogeneous_Method.reverse_mapping import build_reverse_mapping, reverse_map_rule
...
def main(args):
    ...
    try:
        ...
        # --- NEW: Scan entire dataset to compute min/max for mapping generation ---
        logger.info("--- Scanning entire dataset to compute feature ranges for mapping ---")
        ...
        try:
            scan_iterator = pd.read_csv(file_path, chunksize=args.chunk_size, low_memory=False)
            for scan_chunk in scan_iterator:
                scan_chunk_count += 1
                scan_total_rows += len(scan_chunk)
                
                # Apply time transfer to match preprocessing
                processed_scan_chunk = time_scalar_transfer(scan_chunk.copy(), args.file_type)
                scan_features = processed_scan_chunk.drop(columns=label_cols_to_exclude, errors='ignore')
                
                # Update min/max for each numeric feature
                for col in scan_features.columns:
                    try:
                        numeric_vals = pd.to_numeric(scan_features[col], errors='coerce').dropna()
                        if not numeric_vals.empty:
                            if col not in feature_mins:
                                feature_mins[col] = numeric_vals.min()
                                feature_maxs[col] = numeric_vals.max()
                            else:
                                feature_mins[col] = min(feature_mins[col], numeric_vals.min())
                                feature_maxs[col] = max(feature_maxs[col], numeric_vals.max())
                    except Exception:
                        continue
                
                # Progress logging
                if scan_chunk_count % 10 == 0:
                    logger.info(f"  Scanned {scan_chunk_count} chunks ({scan_total_rows} rows)...")
            
            logger.info(f"Scan complete: {scan_chunk_count} chunks ({scan_total_rows} rows), {len(feature_mins)} numeric features found.")
            if feature_mins:
                sample_cols = list(feature_mins.keys())[:5]
                logger.debug(f"Sample feature ranges (first 5): {[(col, feature_mins[col], feature_maxs[col]) for col in sample_cols]}")
        except Exception as e:
            logger.warning(f"Failed to scan entire dataset: {e}. Falling back to first chunk only.")
            feature_mins = {}
            feature_maxs = {}
        
        # --- Load first chunk for structure ---
        try:
            # Read first chunk for structure and non-numeric features
        ...
        # Generate mapping features from first chunk
        mapping_features = processed_first_chunk.drop(columns=label_cols_to_exclude, errors='ignore')
        
        # --- NEW: If we scanned the entire dataset, add min/max bounds to first chunk ---
        # Strategy: Add explicit min/max rows to first chunk data (same approach as best_clustering_selector)
        # This ensures pd.qcut sees the full range without distorting the data distribution
        if feature_mins and feature_maxs:
            logger.info("Adding full dataset min/max bounds to first chunk for mapping generation...")
            
            # Create a copy of first chunk data
            mapping_features_expanded = mapping_features.copy()
            
            # For each numeric feature, add rows with min and max values if not already present
            rows_added = 0
            base_row = mapping_features.iloc[0].copy() if len(mapping_features) > 0 else pd.Series()
            
            for col in feature_mins.keys():
                if col in mapping_features.columns:
                    try:
                        numeric_col = pd.to_numeric(mapping_features[col], errors='coerce').dropna()
                        if not numeric_col.empty:
                            existing_min = numeric_col.min()
                            existing_max = numeric_col.max()
                            full_min = feature_mins[col]
                            full_max = feature_maxs[col]
                            
                            # Only add min if it's significantly different from existing min
                            if abs(existing_min - full_min) > 1e-6:
                                min_row = base_row.copy()
                                min_row[col] = full_min
                                mapping_features_expanded = pd.concat([mapping_features_expanded, min_row.to_frame().T], ignore_index=True)
                                rows_added += 1
                                logger.debug(f"  Added min boundary for {col}: {full_min:.4f} (existing: {existing_min:.4f})")
                            
                            # Only add max if it's significantly different from existing max
                            if abs(existing_max - full_max) > 1e-6:
                                max_row = base_row.copy()
                                max_row[col] = full_max
                                mapping_features_expanded = pd.concat([mapping_features_expanded, max_row.to_frame().T], ignore_index=True)
                                rows_added += 1
                                logger.debug(f"  Added max boundary for {col}: {full_max:.4f} (existing: {existing_max:.4f})")
                    except Exception as e:
                        logger.debug(f"  Failed to add bounds for column {col}: {e}")
                        continue
            
            mapping_features = mapping_features_expanded
            logger.info(f"Added {rows_added} boundary rows to ensure full range coverage ({len(mapping_features)} total rows).")
        
        # Generate mapping rules from the expanded representative dataset (or original if scan failed)
        logger.info("Generating mapping from representative dataset...")
        ...
        logger.info(f"Successfully generated new mapping with {len(category_mapping['interval'].columns)} features (based on full dataset range).")
        
        # --- NEW: Save mapping to CSV (same format as convert_mapping_pickle_to_csv.py) ---
        try:
            # Import convert function (or implement inline)
            from convert_mapping_pickle_to_csv import convert_category_mapping_to_csv
            
            # Determine output path for mapping CSV
            csv_output_dir = os.path.join("../Dataset_Paral/signature", args.file_type)
            ...
            logger.info(f"Saving mapping to CSV: {csv_output_path}")
            logger.debug(f"Category mapping keys: {list(category_mapping.keys()) if category_mapping else 'None'}")
            logger.debug(f"Interval mapping type: {type(category_mapping.get('interval'))}, empty: {category_mapping.get('interval').empty if isinstance(category_mapping.get('interval'), pd.DataFrame) else 'N/A'}")
            
            success = convert_category_mapping_to_csv(category_mapping, csv_output_path)
            if success:
                logger.info(f"Mapping CSV saved successfully to: {csv_output_path}")
                # Verify file was created
                if os.path.exists(csv_output_path):
                    file_size = os.path.getsize(csv_output_path)
                    logger.info(f"Verified: CSV file exists at {csv_output_path} (size: {file_size} bytes)")
                else:
                    logger.error(f"ERROR: CSV file was not created at {csv_output_path}")
            else:
                logger.error("Failed to save mapping CSV: convert_category_mapping_to_csv returned False. Continuing with execution.")
        except ImportError as e:
            logger.error(f"Could not import convert_category_mapping_to_csv: {e}. Mapping CSV will not be saved.")
            import traceback
            logger.debug(traceback.format_exc())
        except Exception as e:
            logger.error(f"Failed to save mapping CSV: {e}. Continuing with execution.")
            import traceback
            logger.debug(traceback.format_exc())
        ...
        for chunk in main_chunk_iterator:
            ...
            try:
                ...
                if not anomalous_data_in_chunk.empty:
                    ...
                    # Strategy 1: Use dominant columns that have DIFFERENT values in normal vs anomaly data
                    # Strategy 2: If no distinguishing columns, check if current turn's anomaly:normal ratio >= 95:5
                    is_fallback_rule = False
                    if (not standard_rules or len(standard_rules) == 0) and dominant_cols:
                        # Check which dominant columns can distinguish normal from anomaly by value difference
                        distinguishing_dominant_cols = {}
                        if not normal_data_in_chunk.empty:
                            normal_dominant_cols = get_dominant_columns(normal_data_in_chunk, freq_threshold=args.dominant_freq_threshold)
                            
                            for col, anomaly_value in dominant_cols.items():
                                if col in normal_data_in_chunk.columns and col in normal_dominant_cols:
                                    normal_value = normal_dominant_cols[col]
                                    # If dominant values differ between normal and anomaly, this column can distinguish
                                    if normal_value != anomaly_value:
                                        distinguishing_dominant_cols[col] = anomaly_value
                                        logger.debug(f"[FallbackRule] Column '{col}' can distinguish: normal={normal_value}, anomaly={anomaly_value}")
                                elif col not in normal_dominant_cols:
                                    # If column is not dominant in normal data, it might still be useful
                                    # Check if it has different values in normal vs anomaly
                                    if col in normal_data_in_chunk.columns:
                                        normal_unique = normal_data_in_chunk[col].value_counts(dropna=False)
                                        if not normal_unique.empty:
                                            normal_top_value = normal_unique.index[0]
                                            normal_top_ratio = normal_unique.iloc[0] / len(normal_data_in_chunk)
                                            # If normal has a different top value or low frequency, use it
                                            if normal_top_value != anomaly_value or normal_top_ratio < 0.8:
                                                distinguishing_dominant_cols[col] = anomaly_value
                                                logger.debug(f"[FallbackRule] Column '{col}' can distinguish: normal_top={normal_top_value} (ratio={normal_top_ratio:.3f}), anomaly={anomaly_value}")
                        
                        if distinguishing_dominant_cols:
                            # Strategy 1: Use columns with different values
                            standard_rules = build_rules_from_dominant(distinguishing_dominant_cols)
                            is_fallback_rule = True
                            logger.info(f"[DominantCols] Created {len(standard_rules)} fallback rule(s) from {len(distinguishing_dominant_cols)} distinguishing dominant columns (out of {len(dominant_cols)} total). These can distinguish normal from anomaly.")
                        elif dominant_cols and not normal_data_in_chunk.empty:
                            # Strategy 2: Check if current turn's ratio (anomaly:normal) >= 95:5 for the dominant rule
                            # Create a test rule from all dominant cols and check its match ratio in current turn
                            test_rule = dict(dominant_cols)
                            
                            # Check matches in anomaly data (current turn)
                            anomaly_matches = 0
                            anomaly_total = len(anomalous_data_in_chunk)
                            if anomaly_total > 0:
                                rule_keys_in_anomaly = [k for k in test_rule.keys() if k in anomalous_data_in_chunk.columns]
                                if rule_keys_in_anomaly:
                                    rule_series_anomaly = pd.Series({k: test_rule[k] for k in rule_keys_in_anomaly})
                                    anomaly_match_mask = anomalous_data_in_chunk[rule_keys_in_anomaly].eq(rule_series_anomaly).all(axis=1)
                                    anomaly_matches = anomaly_match_mask.sum()
                            
                            # Check matches in normal data (current turn)
                            normal_matches = 0
                            normal_total = len(normal_data_in_chunk)
                            if normal_total > 0:
                                rule_keys_in_normal = [k for k in test_rule.keys() if k in normal_data_in_chunk.columns]
                                if rule_keys_in_normal:
                                    rule_series_normal = pd.Series({k: test_rule[k] for k in rule_keys_in_normal})
                                    normal_match_mask = normal_data_in_chunk[rule_keys_in_normal].eq(rule_series_normal).all(axis=1)
                                    normal_matches = normal_match_mask.sum()
                            
                            # Calculate ratio: anomaly_matches : normal_matches
                            # If anomaly_matches >= 95% of total matches, accept the rule
                            total_matches = anomaly_matches + normal_matches
                            if total_matches > 0:
                                anomaly_ratio = anomaly_matches / total_matches
                                if anomaly_ratio >= 0.95:  # anomaly:normal >= 95:5
                                    standard_rules = build_rules_from_dominant(dominant_cols)
                                    is_fallback_rule = True
                                    logger.info(f"[DominantCols] Created {len(standard_rules)} fallback rule(s) from {len(dominant_cols)} dominant columns based on current turn ratio. "
                                               f"Anomaly matches: {anomaly_matches}/{anomaly_total}, Normal matches: {normal_matches}/{normal_total}, "
                                               f"Anomaly ratio: {anomaly_ratio:.4f} (>= 95%).")
                                else:
                                    # Ratio not high enough, still create but will be filtered out
                                    standard_rules = build_rules_from_dominant(dominant_cols)
                                    is_fallback_rule = True
                                    logger.warning(f"[DominantCols] Created {len(standard_rules)} fallback rule(s) from {len(dominant_cols)} dominant columns, "
                                                 f"but current turn ratio is {anomaly_ratio:.4f} (anomaly:normal = {anomaly_matches}:{normal_matches}), "
                                                 f"which is < 95%. This rule will likely be rejected during filtering.")
                            else:
                                # No matches at all, create but will be filtered
                                standard_rules = build_rules_from_dominant(dominant_cols)
                                is_fallback_rule = True
                                logger.warning(f"[DominantCols] Created {len(standard_rules)} fallback rule(s) from {len(dominant_cols)} dominant columns, "
                                             f"but no matches found in current turn. This rule will likely be rejected during filtering.")
                        elif dominant_cols:
                            # No normal data available, create fallback rule
                            standard_rules = build_rules_from_dominant(dominant_cols)
                            is_fallback_rule = True
                            logger.warning(f"[DominantCols] Created {len(standard_rules)} fallback rule(s) from {len(dominant_cols)} dominant columns, but no normal data available for comparison. "
                                         f"This rule will likely be rejected during filtering.")
                    if standard_rules:
                ...
                if rule_spooler.has_rules():
                    ...
                    def rule_passes_global(rule, is_fallback=False):
                        ...
                        if is_fallback:
                            ...
                            # Strategy 1: If both supports are very high (>= 0.95), check current turn's ratio
                            # Strategy 2: If ratio (anomaly:normal) >= 95:5 in current turn, accept even if both supports are high
                            if normal_support >= 0.95 and anomaly_support >= 0.95:
                                # Both supports are high, but check current turn's ratio
                                # Calculate matches in current turn (not cumulative)
                                current_turn_anomaly_matches = 0
                                current_turn_normal_matches = 0
                                
                                if not anomalous_data_in_chunk.empty:
                                    rule_keys_anomaly = [k for k in rule.keys() if k in anomalous_data_in_chunk.columns]
                                    if rule_keys_anomaly:
                                        rule_series_anomaly = pd.Series({k: rule[k] for k in rule_keys_anomaly})
                                        current_turn_anomaly_match_mask = anomalous_data_in_chunk[rule_keys_anomaly].eq(rule_series_anomaly).all(axis=1)
                                        current_turn_anomaly_matches = current_turn_anomaly_match_mask.sum()
                                
                                for chunk in normal_data_chunks:
                                    if chunk is None or chunk.empty:
                                        continue
                                    rule_keys_normal = [k for k in rule.keys() if k in chunk.columns]
                                    if rule_keys_normal:
                                        rule_series_normal = pd.Series({k: rule[k] for k in rule_keys_normal})
                                        current_turn_normal_match_mask = chunk[rule_keys_normal].eq(rule_series_normal).all(axis=1)
                                        current_turn_normal_matches += current_turn_normal_match_mask.sum()
                                
                                # Check if current turn's ratio is >= 95:5
                                current_turn_total_matches = current_turn_anomaly_matches + current_turn_normal_matches
                                if current_turn_total_matches > 0:
                                    current_turn_anomaly_ratio = current_turn_anomaly_matches / current_turn_total_matches
                                    if current_turn_anomaly_ratio >= 0.95:
                                        logger.info(f"[FallbackRule] Accepted: Both supports are high (normal={normal_support:.4f}, anomaly={anomaly_support:.4f}), "
                                                   f"but current turn ratio is {current_turn_anomaly_ratio:.4f} (anomaly:normal = {current_turn_anomaly_matches}:{current_turn_normal_matches}) >= 95%.")
                                        return True
                                
                                # Both supports high and ratio < 95%, reject
                                ratio_to_log = current_turn_anomaly_ratio if current_turn_total_matches > 0 else 0.0
                                logger.warning(f"[FallbackRule] Rejected: Cannot distinguish normal from anomaly. "
                                             ...
                                             f"Current turn ratio: {ratio_to_log:.4f} (anomaly:normal = {current_turn_anomaly_matches}:{current_turn_normal_matches}) < 95%. "
                    ...
                    for rules_batch in rule_spooler.consume_chunks():
                        ...
                        for rule in filtered_rules:
                            ...
                            if rule_id in all_valid_signatures:
                                # Rule already exists in the set
                                # If it's a fallback rule that passed the filter in current turn,
                                # ensure it's not in the blacklist (remove if present)
                                if is_fallback_rule and rule_id in signatures_to_remove:
                                    signatures_to_remove.remove(rule_id)
                                    removed_from_blacklist += 1
                                    logger.info(f"[RuleAdd] Removed existing fallback rule from blacklist (was FP previously, but valid for current turn): rule_id={rule_id}")
                                else:
                                    skipped_already_in_set += 1
                                    if turn_counter <= 2 or is_fallback_rule:  # Log for first few turns or fallback rules
                                        logger.debug(f"[RuleAdd] Skipped rule (already in set): rule_id={rule_id}, rule={dict(list(rule.items())[:3])}...")
                            elif rule_id in signatures_to_remove:
                                # If this rule passed the filter (especially fallback rules with ratio >= 95%),
                                # it means it's valid for the current turn even if it was previously marked as FP.
                                # Remove it from blacklist and add it to allow re-evaluation.
                                if is_fallback_rule:
                                    signatures_to_remove.remove(rule_id)
                                    removed_from_blacklist += 1
                                    logger.info(f"[RuleAdd] Removed fallback rule from blacklist (was FP previously, but valid for current turn): rule_id={rule_id}")
                                    # Now add it to the set
                                    all_valid_signatures[rule_id] = rule
                                    signature_turn_created[rule_id] = turn_counter
                                    new_signatures_found += 1
                                else:
                                    # For non-fallback rules, keep them in blacklist if they were previously marked as FP
                                    if turn_counter <= 2:  # Log for first few turns
                                        logger.warning(f"[RuleAdd] Skipped rule (in blacklist): rule_id={rule_id}, rule={dict(list(rule.items())[:3])}...")
                            else:
                                all_valid_signatures[rule_id] = rule
                    ...
                    if skipped_already_in_set > 0 or removed_from_blacklist > 0:
                        logger.info(f"Added {new_signatures_found} new unique signatures. "
                                   f"Skipped: {skipped_already_in_set} already in set, Removed from blacklist: {removed_from_blacklist}.")
                    else:
                        logger.info(f"Added {new_signatures_found} new unique signatures.")
                else:
        ...
        for sig_idx, (sig_id, rule) in enumerate(sorted_final_signatures, 1):
            ...
            # Reverse-mapped report (with original intervals/values)
            reversed_rule = reverse_map_rule(rule, reverse_mapping)
        ...
        final_signatures_df_reversed.to_csv(output_path_reversed, index=False)
